...
async def encode_jwt(payload: dict) -> str:
    """
...
async def decode_jwt(token: str) -> Optional[dict]:
    """
...
async def verify_password_reset_token(token: str) -> Optional[EmailStr]:
    """
    ...
    :rtype: EmailStr
    """
    decoded_token: dict = await decode_jwt(token)
    if decoded_token:
